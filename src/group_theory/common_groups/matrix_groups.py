...
def PGL(n: int, q: int) -> LinearGroup:
    ...
    PGLnq = LinearGroup((GL / Z).elements)
...
def PSL(n: int, q: int) -> LinearGroup:
    ...
    PSLnq = LinearGroup((SL / Z).elements)
...
def PSO(n: int, q: int) -> LinearGroup:
    ...
    PSOnq = LinearGroup((SO / Z).elements)
...
def _tuple_to_matrix(
    dimension: int, GF: Type[galois.FieldArray], t: tuple
) -> galois.FieldArray:
    """
