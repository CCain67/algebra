...
class Group:
    ...
    def subgroup_generated_by(self, generators: Iterable[GroupElement]) -> Subgroup:
...
class Subgroup(Group):
    ...
    def validate_subgroup(self) -> bool:
        """Cehcks whether or not the elements provided subgroup satisfies the group axioms.

        Returns:
            bool: returns True if the element list satisfies the group axioms, returns
            False otherwise.
        """
        for g in self:
            for h in self:
    ...
    def __eq__(self, other):
        return (self.order == other.order) and all(
            (g in other for g in self.generators)
        )

    def __ne__(self, other):
        return (self.order != other.order) or any(
            (g not in other for g in self.generators)
        )
